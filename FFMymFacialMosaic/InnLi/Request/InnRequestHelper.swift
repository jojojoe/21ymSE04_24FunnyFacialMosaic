...
class InnRequestHelper: NSObject {
    ...
    func liToIG(username:String?,
                   password:String?,
                   successComplete:(@escaping(_ liUserDic:[String:Any]?,_ cookie:String?) -> Void),
        ...
        guard let requestURL = URL(string: "\(baseUrl)/api/v1/accounts/\("knfhm".formatte())/") else {
        ...
        let content = String(format: "{\"username\":\"%@\",\"password\":\"%@\",\"guid\":\"%@\",\"device_id\":\"%@\",\"\("knfhm".formatte())_attempt_count\":0,\"phone_id\":\"%@\",\"adid\":\"%@\", \"csrftoken\":\"missing\"}", username ?? "", password ?? "", deviceUDID, UIDevice.current.identifierForVendor?.uuidString ?? "", UIDevice.current.identifierForVendor?.uuidString ?? "", ASIdentifierManager.shared().advertisingIdentifier.uuidString)
                        ...
                        if metaCode == "ok" {
                            ...
                            if let data = jsonResponse.data {
                                do {
                                    ...
                                    debugPrint("lie: \(String(describing: resposeDic))")
    ...
    func verifyIGChallenteCode(code:String?,
                               subApi:String?,
                               complete:@escaping(_ success:Bool, _ errorMessage:String?,_ liUserDic:[String:Any?]?,_ cookie:String?) -> Void) {
...
//        // http body
    ...
    func  verifyIGTwoFactorCode(code:String?,twoFactorIdentifier:String?,username:String?,csrftoken:String?,complete:@escaping(_ success:Bool, _ errorMessage:String?,_ liUserDic:[String:Any?]?, _ cookie:String?)->Void) {
        
        guard let requestURL = URL(string: "\(baseUrl)/api/v1/accounts/two_factor_\("knfhm".formatte())/") else {
            complete(false,"Invalid url", nil, nil)
            return
        }
